...
describe("corrections tests", () => {
  ...
  it("should correct the wrong hash and mint", async () => {
    ...
    await program.methods
      .addEpoch(epoch1Bump, wrongTreeRoot)
      .accounts({
        rewardsAccount: rewardsAccountKeypair.publicKey,
      ...
      .rpc();
    const initialEpochAccount = await program.account.epochAccount.fetch(
      epoch1
    );
    expect(initialEpochAccount.hash).to.deep.equal(
      wrongTreeRoot,
      "Root hash should be the wrong one"
    );
    assert(
    ...
    // Correct epoch
    await program.methods
      .correctEpoch(new anchor.BN(1), correctTreeRoot)
      .accounts({
        agent: agentKeypair.publicKey,
        mint: correctMint,
        rewardsAccount: rewardsAccountKeypair.publicKey,
    ...
    const correctedEpochAccount = await program.account.epochAccount.fetch(
      epoch1
    );
    expect(correctedEpochAccount.hash).to.deep.equal(
      correctTreeRoot,
      "Root hash should be the correct one"
    );
    assert(
