...
describe("claim tests", () => {
  ...
  // Decimals for the mint used in this test suite
  const decimals = 6;
  ...
  before("create mint and do airdrops", async () => {
    ...
    await mintTo(
      ...
      meKeypair, // Authority
      10_000 * Math.pow(10, decimals), // Amount (scaled for decimals)
      [], // Multi signers
  ...
  beforeEach("initialize", async () => {
    ...
    await program.methods
      .initialize(agentKeypair.publicKey, new anchor.BN(365 * 24 * 60 * 60)) // 365 days withdrawal period
      .accounts({
  ...
  it("should claim from multiple epochs", async () => {
    ...
    // Tree
    const amountUi = new anchor.BN(123.45); // UI tokens
    const scale = new anchor.BN(10).pow(new anchor.BN(decimals));
    const amount = amountUi.mul(scale); // Base units for Merkle tree and claim
    ...
    const proof = tree
      .getProof(leafIndex.toNumber(), receiverKeypair.publicKey, amount)
      .map((e) => Array.from(Uint8Array.from(e)));
    ...
    // Approve epoch 1
    await program.methods
      .approveEpoch(new anchor.BN(1), amount)
      .accounts({
        rewardsAccount: rewardsAccountKeypair.publicKey,
        epochAccount: epoch1,
        manager: managerKeypair.publicKey,
    ...
    // Approve epoch 2
    await program.methods
      .approveEpoch(new anchor.BN(2), amount)
      .accounts({
        rewardsAccount: rewardsAccountKeypair.publicKey,
        epochAccount: epoch2,
        manager: managerKeypair.publicKey,
